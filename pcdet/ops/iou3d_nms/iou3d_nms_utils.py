...
def niv_nms_gpu(boxes_preds, src_scores_preds, thresh, pre_maxsize=None, norm_size=6.0, niv_weight=1.0, **kwargs):
    
    ans_iou = boxes_iou_bev(boxes_preds, boxes_preds)
    select = ans_iou > 0.2
    
    iou_voting_nums = select.sum(dim=1).type_as(ans_iou)
    ...
    return keep_idx, niv_scores_preds[keep_idx]
