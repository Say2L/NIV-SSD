...
class DataAugmentor(object):
    ...
    def points_sampling_aug(self, data_dict=None, config=None):
        ...
        if data_dict is None:
            return partial(self.points_sampling_aug, config=config)

        """gt_boxes = deepcopy(data_dict['gt_boxes'])[data_dict['gt_boxes_mask']]
        points = deepcopy(data_dict['points'])
        draw_scenes(points, gt_boxes)"""

        """sampling_points, boxes_keep_idxs = augmentor_utils.farthest_point_sampling(
            data_dict['points'], 
            data_dict['gt_boxes'], 
            keep_range=config.get('KEEP_RANGE', [0.5, 1.0]),
        )"""
        sampling_points, boxes_keep_idxs = augmentor_utils.points_down_sampling(
            data_dict['points'], 
            data_dict['gt_boxes'], 
            dist_ranges=config.get('DIST_RANGES', [20, 40]),
            keep_ranges=config.get('KEEP_RANGES', [[0.5, 1.0], [0.6, 1.0], [0.8, 1.0]]),
        )
        data_dict['gt_boxes'] = data_dict['gt_boxes'][boxes_keep_idxs]
        data_dict['gt_names'] = data_dict['gt_names'][boxes_keep_idxs]
        data_dict['gt_boxes_mask'] = data_dict['gt_boxes_mask'][boxes_keep_idxs]
        data_dict['points'] = sampling_points
